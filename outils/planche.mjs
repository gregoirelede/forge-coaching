...
const VAR = [["A-sable-approfondi", "A — Sable approfondi"],
             ["B-vert-ancrage",     "B — Vert d'ancrage"],
             ["C-sable-chaud",      "C — Sable chaud"]];

const b = await chromium.launch();
for (const [ecran, theme, titre] of [
  ...
  const ctx = await b.newContext({ viewport: { width: 1360, height: 900 }, deviceScaleFactor: 1.6 });
  const p = await ctx.newPage();
  await p.setContent(html, { waitUntil: "load" });
  await p.waitForTimeout(500);
  const nom = `${SORTIE}/planche-${ecran}-${theme}.png`;
